  ...
  $.ig.angular.igCombo.bindEvents = $.ig.angular.igCombo.bindEvents || function (scope, element, attrs, model) {
    ...
    element.on($.ig.angular.igCombo.events.join(' '), function (event, args) {
      scope.$apply(function () {
        //apply all selected items to model for multiselect combo
        if (args.owner.options.dataIgControlName === 'igCombo' && args.owner.options.multiSelection === 'on') {
          model.$setViewValue(args.owner.options.selectedItems);
        }
        else {
